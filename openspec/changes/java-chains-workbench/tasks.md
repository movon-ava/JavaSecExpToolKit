...
## 10. 界面收尾：启动预热、选链区比例、布局期分割线（2026-09-22）

写入域：`src/ui/**`、`src/Main.java`（角色：ui）

用户指令：「完善 UI，利用链选择框太小，要和 JavaChains 一样的格式」「软件启动的时候将需要
进行加载初始化的东西都做好」「恶意服务器 / payload 第一次点进去会卡几秒」，
收尾澄清为「并不是和网页版一样高，而是和网页版中的**选链区和整个网页的比例**要一样」。

- [x] 10.1 先量出网页版的权威数值再动手，不接受估算：用 CDP 驱动无头 Chrome 打开上游
  `#/Generate/<payloadKey>` 页读 `getBoundingClientRect()`，得控制台 `.studio-top-console`
  360px、选链区 `.chain-builder-block` 486px、候选列表 `.option-list` 320px；
  验证：实测输出写入 `docs/DESIGN-payload.md` 第 11.2 节
- [x] 10.2 上下分栏对齐**比例**而非绝对值：`CONSOLE_WEIGHT` 由上述两个实测常量算出
  （选链区占两块之和约 57.4%）；验证：`Ratio` 工具实测选链区 462px / 56.5%，与网页版 57.4% 同量级
- [x] 10.3 一次性初始化挪到启动阶段并保持幂等：引擎 / 预设链 / 服务适配器在后台线程预热，
  二次调用 0 ms；验证：预热首次 1263 ms、二次 0 ms、预设 52 条
- [x] 10.4 修掉「同一构建读到两个分割线位置」的根因：比例改为在 `doLayout()` 里校正，
  不再依赖异步组件事件；验证：首次读 `divider` 即为 322px（原为 290px），
  `UiNavigationCheck` 的「输出区可视高度 > 120px」由失败转为通过
- [x] 10.5 长提示与状态文字改用按宽度折行的标签，避免窄栏里被截成省略号；
  验证：`getText()` 仍返回原始文本，既有 `startsWith("生成成功")` 一类断言不受影响
- [x] 10.6 八页布局审计无遮挡错位；验证：`LayoutAudit` 扫描 `TOTAL: 0`
- [x] 10.7 六套 Java 自检与 Python 单测全绿，断言数只增不减；
  验证：225 / 68 / 45 / 36 / 26 / 94 条断言失败均为 0，Python 106 项 OK
- [x] 10.8 界面文件规模仍在上限内；验证：`src/Main.java` 327 行（≤400），
  `src/ui/` 内最大 526 行（≤600），由 `tests/test_decoupling.py` 机械校验
