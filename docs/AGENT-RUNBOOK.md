...
| 段落 | 内容 |
| --- | --- |
| 本次调用的角色 | 本次实际启动的角色清单（**中文显示名**，括号内附角色标识），以及各自占用第几步 |
| 各角色主要工作 | 每个角色一句任务描述 + 该步实际改动的文件清单 |
| 逐步结果 | 逐步的状态、说明、提交记录（诊断用） |

汇报里的角色一律写成 `主 agent（orchestrator）` 这种**显示名（标识）**形式。
两套名字各有用途，不要互相替代：

| 形式 | 例子 | 用在哪 |
| --- | --- | --- |
| 角色标识 | `probe` / `traffic` / `ui` | 分支名、worktree 目录、写入域、`-Role` 参数 |
| 中文显示名 | `探测 agent` / `抓包 agent` / `界面 agent` | 面向人的汇报输出 |

映射表在 `tools/lib/RoleMatrix.ps1` 的 `Get-RoleDisplayNames`；未登记的角色原样回退成标识，
不会让汇报丢信息。工具链自检断言「全部角色都有中文显示名」「显示名不等于标识」
「派发与编排两处汇报都用显示名」。
