...
- 第九节修正三提到的「`src/ui/` 下 32 个文件」已增至 35 个：Payload 三件套越过
  「单文件 ≤ 600 行」后按职责拆出 7 个协作类，仍全部平铺在 `ui` 包内、边界规则零改动。
- `Main.java` 由 311 行微调至 301 行；第十节的断言数与 Python 测试数已被后续变更刷新
  （最新值见 `PROGRESS.md`），此处保留为当时快照。
