...
> 添加一个日志记录功能，方便排错，按日期分开，定期清理

拆成三件事：**按日期分文件**（一天的现场进一个文件，跨天自动换）→ **定期清理**（保留最近 N 天，
启动与跨天各清一次）→ **可配置**（开关 / 级别 / 目录 / 保留天数五项落进配置页）。

日志写在共享内核（`src/util/`）而不是界面层：本仓库的依赖方向被约束为单向，功能模块之间不许互相依赖，
因此「任意模块都要能调用」的东西只能沉到被所有层依赖的那一层。

### 二、根因分析（改代码之前先定位成因）

排错困难不是「没写日志」这么笼统。翻代码后定位到三个具体缺口，共同点是**都不会报错，只会让现场消失**。

**缺口 1：异常被静默吞掉。** 全仓库 119 处 `catch` 中有大量以「忽略」结尾，注释写的理由几乎都是同一个：
「不影响主流程」。这个判断本身没错——单条代理连接失败确实不该拖垮整个代理，某个节点没有后继也确实是正常结论。
问题出在处理方式：**把异常丢掉，等于把「失败过」这件事一起丢掉了**。于是使用者的体验是「点了按钮没反应」，
而不是「报错」。

**缺口 2：后台线程的异常无处可去。** 代理的接受循环、隧道泵、引擎与脚本的输出读取都不在事件分发线程上。
这些线程抛出的异常默认只打到标准错误；从图形界面启动时标准错误没有接收方，等于凭空消失。
使用者报告现象时，现场早已不存在。

**缺口 3：没有时间基准。** 即使事后从别处拿到线索，也无法回答「这个问题是今天出现的，还是一周前就有」——
没有任何按时间落盘的东西可查。

具体到一个已存在的例子：代理转发到上游失败时，转发逻辑把异常包成 `IOException` 抛给上层，
上层又吞掉并关闭连接。使用者看到「浏览器访问超时」，而真实原因（上游连不上、目标端口写错）**被丢弃了两次**。

因此本轮修法是**保留既有判断、补上记录**，而不是改行为：所有「不影响主流程」的结论一律保留，
只在旁边补一行日志。判定逻辑一行未改。
...
| 文件 | 写入域 | 动作 |
| --- | --- | --- |
| `src/util/LogFiles.java` | 主 agent | 新增：纯规则层——文件名拼接与反解、保留天数判定、清理（三道闸） |
| `src/util/Log.java` | 主 agent | 新增：记录通道——级别过滤、时间戳、异常栈展开、跨天切换、未捕获异常入库 |
| `src/ui/LoggingBootstrap.java` | 界面 | 新增：配置语义层，把配置页五项翻译成一次 `Log.configure` |
| `src/proxy/ProxyServer.java` | 抓包 | 改：启动 / 停止留痕；监听失败、接受连接失败、隧道中断、上游连接失败、转发失败、抓包回调异常逐处记录 |
| `src/payload/PayloadEngine.java` | 利用链 | 改：初始化失败与构建失败记 error，节点查询 / 元信息 / 指标读取等失败记 debug |
| `src/shiro/ShiroEngine.java` | 利用链 | 改：检测请求失败、字典读取、密钥确认、序列化、加解密、连通性、GCM 回退、爆破中断逐处记录 |
| `src/analyzer/EngineRunner.java` | 利用链 | 改：超时警告附上限秒数、启动失败、输出流结束、临时目录清理失败 |
| `src/analyzer/ScriptRunner.java` | 利用链 | 改：超时、启动失败、中断、输出流结束 |
| `src/analyzer/ProcessTree.java` | 利用链 | 改：进程已退出 / 强杀失败 / 强杀异常 |
| `src/analyzer/Decompiler.java` | 利用链 | 改：输出目录创建失败、整包与单类反编译失败、产物统计失败 |
| `src/analyzer/VulnerabilityAnalyzer.java` | 利用链 | 改：跳过损坏 jar 由静默改为 warn（含异常类型与原因） |
| `src/analyzer/DependencyScanner.java` | 利用链 | 改：读取 jar 条目失败记 debug |
| `src/ui/StartupWarmup.java` | 界面 | 改：单步预热失败记 error，汇总结论记 info |
| `src/ui/WorkbenchPages.java` | 界面 | 改：关闭恶意服务器失败由静默改为 warn |
| `src/ui/ConfigForm.java` | 界面 | 改：新增 5 个控件与「日志配置」分组 |
| `src/ui/ConfigController.java` | 界面 | 改：级别四档映射、`resetForm()` 回填、`save()` 落盘 5 键，保存后立刻重配日志并回显状态 |
| `src/ui/WidgetRegistry.java` | 界面 | 改：登记 5 个日志配置控件 |
| `src/Main.java` | 界面 | 改：构造期用默认值开日志 → 读配置 → 按配置重开；`main` 首行安装未捕获异常处理器 |
| `tests/LogCheck.java` | 测试 | 新增：79 条断言 |
| `tests/test_logging.py` | 测试 | 新增：26 项单测（规则层独立实现 + 实现契约） |
| `tests/UiNavigationCheck.java` | 测试 | 改：新增 9 条日志配置断言 |
| `tests/test_decoupling.py` / `tools/audit_boundary.py` | 测试 | 改：边界规则新增「功能层叶子」档（两份副本同步） |
| `tests/test_selfcheck_hygiene.py` | 测试 | 改：自检入口清单补 `LogCheck.java` |
| `docs/DESIGN-logging.md` | 主 agent | 新增：设计文档 |
| `README.md` / `README.en.md` | 主 agent | 改：配置页 / 项目结构 / 测试章 / 新增「日志记录」小节，中英同步 |
| `PROGRESS.md` / `AI_REPORT.md` | 主 agent | 改：状态表与时间线 |
| `openspec/changes/{logging-core,logging-integration,ui-logging-config}/**` | 主 agent | 新增后归档，主 spec 落地三份 |

### 四、功能行为

- **按日期分文件**：文件名固定为 `app-YYYY-MM-DD.log`，一个月只有一个文件对应一天，跨天写入时自动切换。
  长时间开着程序不会把日志堆进同一个文件。
- **定期清理**：默认保留最近 7 天（**含今天**），启动时清一次、跨天换文件时再清一次。
  「含今天」是刻意的：语义写成「N 天前开始保留」会多留一天，与使用者直觉不符。
- **默认开启**：排错能力不该需要先手动打开。关掉后不再产生新文件，已有日志保留。
- **目录解析**：环境变量 `JSETK_LOG_DIR` > 配置页填写值 > 用户目录 `~/.JavaSecExpToolKit/logs`。
  环境变量优先是为了自检与便携场景能在不改配置的情况下把日志引走。
- **级别四档**：`仅错误` / `警告与错误` / `常规`（默认）/ `调试`。线索不足时改「调试」，
  重跑一次复现路径即可；平时保持「常规」以免文件迅速膨胀。
- **未捕获异常统一入库**：后台线程（代理接受循环、隧道泵、引擎输出读取）的异常由
  `Thread.setDefaultUncaughtExceptionHandler` 写入日志，不再只打标准错误。
- **写入失败静默**：日志是旁路，写不进去（目录不可写、磁盘满）时丢弃这一条，绝不影响主流程。

配置页新增「日志配置」分组 5 键：`log_enabled`、`log_level`、`log_dir`、`log_keep_days`、`log_console`。
保存后立刻重新初始化并回显一行状态（目录 / 级别 / 保留天数 / 本次清理了几个过期文件）。

### 五、验证记录

| 验证项 | 命令 | 结果 |
| --- | --- | --- |
| 日志内核自检 | `LogCheck` | exit 0，**79** 条断言，失败 0 |
| 载荷自检 | `PayloadCheck` | exit 0，100 条断言，失败 0 |
| 漏洞分析自检 | `AnalyzeCheck` | exit 0，61 条断言，失败 0 |
| 其余五套自检 | `ProxyServerCheck` / `ShiroCheck` / `UiShiroCheck` / `UiNavigationCheck` / `UiSwitchEndToEndCheck` | 全部 exit 0 |
| Python 单测 | `python -X utf8 -m unittest discover -s tests` | **Ran 156 tests OK**（原 130 + 新增 26） |
| 依赖边界 | `python -X utf8 tools\audit_boundary.py` | 结论「全部通过」（叶子层出边无、功能层叶子越界无、共享内核反向依赖无） |
| 工具链自检 | `powershell -File tools\check_agent_tools.ps1` | **91 项通过** |
| OpenSpec | `openspec validate --all --strict` | **14 passed, 0 failed** |
| 构建 | `.\build.ps1` | BUILD SUCCESS，JAR 时间 `2026-09-23 10:53:18`，`Source freshness checked: 120 file(s)` |
| 文件规模 | `Main.java` 319 行（≤400）；`src/ui/` 最大 553 行（≤600） | 通过 |

`LogCheck` 覆盖的是最容易写错、又最不该靠造文件去猜的两类逻辑：

| 覆盖点 | 断言内容 |
| --- | --- |
| 文件名解析 | 严格 `yyyy-MM-dd`；月份不补零（`app-2026-9-3.log`）、非法日期（二月三十）一律不认 |
| 保留边界 | 跨月 / 跨年 / 闰年边界；填 0 或负数归一到 1；「含今天」语义 |
| 目录优先级 | 环境变量 > 配置 > 用户目录默认值 |
| 级别过滤 | 四档阈值各自放行与拦截的条目 |
| 跨天切换 | 用可替换的日期来源注入「第二天」，断言新条目进新文件 |
| 清理安全 | 只删本工具命名的文件；目录里的无关文件与子目录一律不碰 |
| 失败静默 | 目录不可写时记录动作不抛异常 |
| 未捕获异常 | 异常栈确实进了日志 |

跨天切换无法靠等待一天来验证，因此把日期做成了可替换来源（`Log#setDateSupplier`），自检注入日期断言文件切换。

### 六、多 Agent 协作

| 角色 | 主要工作 |
| --- | --- |
| 主 agent | 需求澄清与任务拆分、根因定位、内核两层（`LogFiles` / `Log`）与配置语义层落地、共享内核写入、三份 OpenSpec change 规划与归档、设计文档、README 中英同步、构建与门禁总控 |
| 抓包 agent | `ProxyServer` 六处静默失败路径接入（监听 / 接受连接 / 隧道中断 / 上游连接 / 转发 / 抓包回调），保留原有「不影响主流程」判定 |
| 利用链 agent | `PayloadEngine`、`ShiroEngine`、`analyzer` 五个类共十余处失败路径接入，按「使用者能否据此定位」区分 error 与 debug |
| 界面 agent | 配置页 5 个控件与「日志配置」分组、级别四档映射、回填与落盘三处同步、保存后立即重配并回显状态、`WidgetRegistry` 登记 |
| 测试 agent | `LogCheck` 79 条断言、`test_logging.py` 26 项单测、`UiNavigationCheck` 9 条配置页断言、自检入口清单补登记 |
| 监督 agent | 逐文件比对写入域与包级依赖边界，确认边界规则改动是「放宽且可解释」而非删除；核对文件规模上限与断言只增不减 |
...
- 改动前快照 `.backups/20260923-085531`，按规则只保留最近三份。
- `.backups/`、`target/`、`JavaSecExpToolKit.jar`、`.pi/` 由 `.gitignore` 忽略。

### 八、如实说明的局限

1. **日志不含请求体、响应体、密钥与令牌**：只记录事件与结论。需要看报文原文时仍要看抓包页。
   这是刻意的取舍——日志文件通常会被留存、粘贴、上传，塞进报文与密钥等于扩大泄露面。
2. **清理只认本工具命名的文件**：`app-YYYY-MM-DD.log` 且位于日志目录之下的常规文件。
   反向的代价是手工改名或挪到别处的旧日志不会被自动清掉。
3. **目录不可写时静默失败**：记录动作全部生效但落不了盘，功能不受影响，使用者也不会被打断。
   需要日志时用环境变量 `JSETK_LOG_DIR` 指向可写目录。
4. **`src/preset/` 保持零出边**：读取内置预设失败时不记录，只把失败原因交给界面 `loadError()` 展示。
   这是包级依赖边界的要求（`preset` 是叶子层），不是遗漏。
5. **本轮只覆盖已知的高频失败路径**：119 处 `catch` 中仍有大量纯「结论型」分支（例如「节点没有后继」
   这类正常结果）未逐条记录，需要时再补。
6. **三份 change 已归档**，但 `analyze-local-deps`、`payload-generation`、`java-chains-workbench`
   仍未归档，待各自功能稳定后再走 archive 流程。
