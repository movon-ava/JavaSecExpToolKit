...
- 可能长时间运行的任务（代码、脚本、agent 会话）必须设超时上限，不得无限期等待；
  超时后要终止任务及其子进程，并保留可判断进度的日志。
- 未完成的产出不得当作可用结果；不得因超时而直接合并。
- 多 Agent 并行时各自在独立 worktree 与分支改码，写入域不得重叠；
  共享资源（备份、报告、进度、构建产物）并发期间一律不碰。详见 `docs/AGENT-RUNBOOK.md`。
