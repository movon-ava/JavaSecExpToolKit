...
- `漏洞分析配置` — 默认扫描目标（jar / 依赖目录）、外部引擎 JAR（`jar-analyzer-engine`，
  ...
  是否在特征匹配后同时导出机器可读 JSON（默认开启）、机器可读报告目录（留空写到后端工作目录下的 `analyze-report.json`）
...
分析目标可以是**单个 jar**，也可以是**依赖目录**（递归取其中全部 jar）；
另外可以给一个源码工程的 `pom.xml`，把「工程声明的依赖」与「产物里的依赖」对照看。

坐标按来源分层采信，并把这个来源一路带到结论里（决定可信度）：

1. `META-INF/maven/**/pom.properties` — 打包时由 Maven 写进去的事实
2. `META-INF/maven/**/pom.xml` — 同样是打包产物，但可能被 shade 改写
3. `META-INF/MANIFEST.MF` — 由打包插件写入，粒度粗
4. 文件名推断 — 只是推测，可信度最低

读取全部走 `JarFile` 的条目枚举：**不解压、不落盘、不加载任何类**。目标 jar 可能是任意来源的
产物，解析期间不得让它获得执行机会。

规则表 25 条，覆盖 fastjson（6 条按 autoType 绕过方式分段）、fastjson2、Shiro（4 条，
含 rememberMe 默认密钥与两处鉴权绕过）、gadget 依赖（CommonsCollections 3 / 4、Groovy、
ROME、CommonsBeanutils、c3p0、Hutool）与组件 RCE（Log4Shell 两段、Spring4Shell、
XStream、SnakeYAML、Jackson-databind、FakeMySQL 场景）。

规则是**本项目自己写的**：上游 `jar-analyzer` 的规则文件是 GPLv3，搬进本仓库有许可问题；
规则内容取自各组件的公开公告，且只收录「判定出来之后本工具能接上后续动作」的那些。
结论按可信度排序，报告末尾固定带一句「没命中不代表没有漏洞」。

每条结论附两个独立维度：**可信度**（来源有多可靠）与**状态**（验证走到哪一步）。
状态取值为 `observed` / `suspected` / `confirmed` / `not-reproduced` / `unknown`，
静态规则命中**一律记 `suspected`**：可信度高不等于漏洞已确认。
两个维度合起来，才不会把「来自 pom 元数据的高可信度」读成「漏洞已确认」。

结论同时回答「**还缺什么**」：报告里有「成立还需要（缺一不可）」与「反证条件」两栏，
以及这条判定依据的**规则来源**。规则表自带版本号与维护日期，报告头会印出来：
没有版本的规则库回答不了「这次结论是哪一版规则算出来的」，也无法在规则更新后判断结论为何变了。

一条结论给出后，内容区顶部会出现**分析传递条**：写明这条结论的来源、目标、命中证据与局限，
并可一键回跳到产生它的分析页。它**只展示、不预填**：类名、常量、推测出的回连地址都不会自动填进
命令、回连地址或目标 URL，不发包、不生成载荷、不启动服务。结论超过 30 分钟后传递条会追加提醒，
建议重新核对后再沿用——否则一段时间之前的判定看起来像刚跑出来的。

引擎调用按**子进程真实退出码**判定成败：此前退出码被丢弃，于是「查询没跑成」与「目标干净」
在界面上长得一模一样。现在只要非零退出码或超时，报告前会冠一段失败横幅，明确写出
「这不是结论、也不代表目标干净」，状态行也不会显示为完成。

实测边界：本地依赖分析单 jar 约 30 ms；建库是分钟级；特征匹配在 65 条特征上约百毫秒量级。
...
它产出的数据库文件固定叫 `jar-analyzer.db` 且**只能落在工作目录**（没有输出路径参数），
所以配置页还有一个 `引擎工作目录`。不填引擎时该页不报错，只是提示去配置页补。

构建大 jar 是分钟级任务，因此有 `调用链超时`（默认 300 秒，低于 30 秒会被提前拦下），
超时会**终止进程树**并清掉临时目录。构建完成后可以反复查询五类内容：

| 查询 | 用途 |
| --- | --- |
| 总览 | 类 / 方法 / 调用边数量，先确认库构建成功 |
| 入口点 | Spring Controller / Servlet / Filter / Listener |
| Sink 命中 | 按内置 sink 清单匹配危险调用（命令执行 / JNDI / 反序列化 / SQL / 文件 / SSRF 等 26 项） |
| 字符串常量 | 检索 SQL、URL、密钥等敏感信息（唯一使用关键字的一类） |
| 组件清单 | 引擎侧口径的组件与版本，可与本地分析结果对照 |
...
每条结论带**状态**与**规则来源**：静态特征命中一律记「疑似」，报告头印出规则库版本与维护日期，
每条结论给出「成立还需要（缺一不可）」与「反证条件」。绕过手法按**本次命中事实筛选**后列出，
不再按漏洞大类整段贴出——技巧清单里多数条目依赖的组件根本不在目标里。

#### 按类型 / 证据来源筛选

特征匹配按钮右侧有三个筛选控件：`严重度`、`证据来源`（全部证据 / 仅 sink 调用 / 仅字符串常量 /
仅类名 / 仅方法名）、`漏洞类型`（全部类型 / 命令执行 / 反序列化 / …）。

筛选做在**脚本侧**而不是界面按行过滤：报告的聚合、绕过手法筛选与「未命中清单」都依赖
「本次参与了哪些特征」这个输入，界面过滤会把命中计数与清单割裂，使用者对不上账。
报告头会写明本次筛选，并说明「未命中清单只覆盖筛选后的特征」。

**未知取值直接报错并列出可选值**，不静默忽略：静默忽略会让「筛完没有结论」看起来像「目标干净」。
筛空时报告也明确写出这不代表签名库里其余特征未命中。

漏洞类型的候选值不在界面写死，而是由签名库通过 `--list-filters` 给出（只读签名库、**不连数据库**，
实测约 90 ms），进页后在后台线程补全；取不到时回落到「不筛选」，而不是筛成空。

#### 同输入重跑比对

`与上次比对`（默认开启）会拿上一次导出的机器可读报告当基线，在报告尾部给出差异：
**新增命中** / **本次不再命中** / **命中数变化**。「这次和上次不一样」本身是重要信号——
可能换了目标，也可能规则库刚更新，只给一份当前结果无从判断结论为何变了。

基线文件不存在、内容损坏或 schema 不同时**回落为不做比对**，本次分析照常成功：
比对是附加信息，缺它不该让整次分析失败。报告会把「没有差异」与「没有基线」分开写。

#### 机器可读导出

勾选配置页的「导出机器可读报告」（默认开启）后，特征匹配会同时写一份 JSON
（默认落到后端工作目录下的 `analyze-report.json`），schema 为 `jsetk.analyze.signatures/1`，
内容含数据源状态、维度说明、局限列表，以及**任务元数据**：工具版本、数据库路径与 SHA-256、
数据库大小与修改时间、生成时间、本次生效的筛选条件。没有这些字段，这份文件只能算「一份结果」，
不能算「可复现的结果」。

导出失败时报告如实写「未写出」并给出路径，不显示为导出成功：写失败却显示成功，
使用者会拿着不存在的文件去对接下游流程。
...
只供使用者用编辑器打开。CFR 是同步 API、无法中途打断，因此超时在这里是
「完成后判定」而不是「中断」——超出预算时结果被标注，产物仍保留。

设计见 [docs/DESIGN-analyze.md](docs/DESIGN-analyze.md)。
...
| `test_signatures.py` | 签名库格式、**sink 特征与 `jar_report.SINKS` 逐条一致**、每种漏洞类型都被指向、按严重度 / 类型 / 证据来源 / 标签筛选与未知取值的失败路径、机器可读导出的任务元数据、同输入基线比对（含基线损坏 / schema 不同的回落）、坏 JSON / 坏正则 / 未登记类型的失败路径 |
| `test_backend_e2e.py` | **真实后端端到端**（P0）：真建库 → 核对表结构 → 跑查询 → 跑特征匹配；后端缺席时**跳过并写明「未验证」而不是通过** |
...
| --- | --- | --- |
| `UiNavigationCheck` | 侧边栏展开 / 收起、各页面控件与端到端流程（含 toString 链页、带外 Jar 页、漏洞分析两页的筛选控件与交接条、新增配置分组），并真实造 jar 跑一次本地依赖分析；断言两页标题与控件集合互不相同（拆页回归守卫），截图写入 `target/ui-check/` | 需要（预设链端到端生成） |
...
| `AnalyzeCheck` | 漏洞分析内核（134 条断言）：版本比较语义（含 `1.2.80 > 1.2.9`、预发布小于正式发布）、四类坐标来源与优先级、规则命中与不命中、group 不符时不误报、端到端分析与跳转建议、外部引擎与数据库的失败路径，以及 gadget 判定（坐标 + 版本区间、已修复版本不误报、新坐标不漏判、外部规则文件语法与失败路径）、结论状态与缺失证据、规则库来源与版本、分析上下文与证据摘取上限、筛选参数拼装与类型下拉映射 | 不需要 |

依赖边界自检、源码级别门禁与工具链自检：

```powershell
python -X utf8 tools\audit_boundary.py        # 包级依赖边界（无环 / 分层 / 通用组件 / 共享内核）
python -X utf8 -m unittest tests.test_java8_source_level  # src/ 不得出现 Java 9+ API 与语法
powershell -File tools\check_agent_tools.ps1  # 多 Agent 工具链 91 项机械断言
```

源码级别门禁这条不是多余的：`src/pom.xml` 把源码级别锁在 release 8，而上面那几条手写的
